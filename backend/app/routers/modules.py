...
def list_modules(department: str | None = Query(None), year: str | None = Query(None), db: Session = Depends(get_db), user: models.User | None = Depends(get_current_user)):
    ...
    if department:
        q = q.filter(models.Module.department_code == department.upper())
    else:
        ...
        if user and user.role != 'admin' and user.department_code:
            ...
            q = q.join(models.Module.departments, isouter=True).filter((models.Module.department_code == code) | (models.Department.code == code))
    if year:
...
def create_module(payload: schemas.ModuleCreate, db: Session = Depends(get_db), _admin=Depends(require_admin)):
    ...
    # create module and associate with departments if provided
    data = payload.model_dump(exclude={"departments"})
    item = models.Module(**data)
    if payload.departments:
        deps = []
        for code in payload.departments:
            d = db.query(models.Department).filter(models.Department.code == code.upper()).first()
            if d:
                deps.append(d)
        if deps:
